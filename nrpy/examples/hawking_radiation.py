"""
Hawking Radiation example:
A modification of Two black holes collide example.


Specifically, evolve Brill-Lindquist initial data forward in time, with one black hole's
mass reduced to zero, and adding a slightly negative density to T_{mu}{nu};
...
# Code-generation-time parameters:
project_name = "hawking_radiation"
CoordSystem = "Spherical"
IDtype = "BrillLindquistHawking"
...
default_BH1_mass = 3.0
default_BH2_mass = 0.0
default_BH1_z_posn = 0.0
default_BH2_z_posn = 0.0
MoL_method = "RK4"
fd_order = 4
...
if parallelization != "cuda":
    ...
    BHaH.BHaHAHA.BHaH_implementation.register_CFunction_bhahaha_find_horizons(
        CoordSystem=CoordSystem, max_horizons=1, main_data_extract=main_data_extract
    )
...
if parallelization == "openmp":
    ...
    par.adjust_CodeParam_default(
        "bah_initial_grid_z_center", [default_BH1_z_posn]
    )
...
BHaH.main_c.register_CFunction_main_c(
    ...
    post_MoL_step_forward_in_time="""for (int grid = 0; grid < commondata.NUMGRIDS; grid++) {
        ...
        double a = -0.01;
        double b = AH_data[8]; //AH_data[8] corresponds to the AH mean radius
        double c = .2;
        for (int k = 0; k < Nxx_plus_2NGHOSTS2;k++) {
            REAL xx2 = griddata[grid].xx[2][k]; 
            for (int j = 0; j < Nxx_plus_2NGHOSTS1;j++) {
                REAL xx1 = griddata[grid].xx[1][j]; 
                for (int i = 0; i < Nxx_plus_2NGHOSTS0;i++) {
                    REAL xx3 = griddata[grid].xx[0][j];

                    const REAL xx[3] = {xx1, xx2, xx3};
                    REAL xCart[3];
                    xx_to_Cart(&griddata[grid].params, xx, xCart);
                    
                    double r = sqrt(xCart[0]*xCart[0] + xCart[1]*xCart[1] + xCart[2]*xCart[2]); 
                    
                    double rhoval =  a*exp(-(r-b)*(r-b)/(2*c));
                    griddata[grid].gridfuncs.auxevol_gfs[IDX4(T4UU00GF, i,j,k)] = rhoval;
                }
            }
        }
        }""",
